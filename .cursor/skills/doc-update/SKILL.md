...
2. 영향받는 docs 영역을 찾는다:
   - 동작 변경 → `documents/domain/README.md` 또는 관련 도메인 문서
   - 아키텍처 변경 → `documents/architecture/README.md`
   - 반복 절차 변경 → `documents/runbooks/`
   - 설계 결정 → `documents/adr/`에 ADR 추가
3. 코드와 문서가 충돌하는 문장을 우선 수정한다.
4. 필요한 경우 ADR 또는 runbook을 추가한다 (`documents/adr/ADR-0000-template.md` 참조).
...
- `@documents/adr/ADR-0000-template.md`
- `@documents/runbooks/bugfix-runbook.md`
- `@AGENTS.md`
