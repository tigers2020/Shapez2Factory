...
- 2026-06-12 — **implement** — Task 6a (HITL approved): policy A — `build_effective_cell_view_index` unions `cell_overlay_json` paint-target rows (Python registry + JS mirror); transport rotation from overlay sources; `test_build_effective_cell_view_index_unions_cell_overlay_json` + green `test_overlay_fallback_frame_includes_pipe_sprite_from_cell_overlay_json`; no harvest delete / flag / NON_SPRITE / tag; commit `7ee4d47e`
- 2026-06-12 — **verify** — Task 6 Step 6.1 (CP-1): D′ flag flip — `labPaintLegacyOptIn()` + `labPaintV2Enabled()` default-on; legacy via `data-lab-paint-legacy="1"` or `data-lab-paint-v2="0"`; contract tests added; gate **30/30** pytest; manual smoke on `copy-import-52921cd2` run #5 frame 38 `(10,7)` — v2 domPlan miner sprite + ring, no full-fill; flag matrix default/legacy/v2-0/restored OK; commit `6f72613b`; no harvest/DOM delete, no template change, no tag
- 2026-06-12 — **align** — Task 6 Step 6.2 (read-only): `equipment_bundles` = **chrome-only + duplicate** for observed runtime (live replay frames 0–3 bundle-only overlay + `map_view.full_cells` covers sprites); **no JS paint index mirror** before delete; reopen only with sparse bundle-only/no-`map_view` repro
- 2026-06-12 — **verify** — Task 6 Step 6.3 (CP-3): harvest canvas delete — `buildCanvasPaintPlan` + preload + terrain filter always paint-plan authority (flag-independent canvas); removed `stageCell`, harvest carry/preload loops; legacy DOM/`labSpriteRelpathForCell` retained; gate **35/35** pytest; commit `93f457c1`
...
- 2026-06-12 — **verify** — Task 6 Step 6.6 (CP-6): `candidate_miner` removed from NON_SPRITE (JS + Python mirror); `isCandidateMinerOverlayKind` resolver/observation guards; deleted `applyLabCellSprite`/`labSpriteRelpathForCell` shims; structural `cellRenderToken` (no harvest sprite); stub/route_path retained; gate **54/54** pytest
