...
def parse_gene_template_record(record: dict[str, Any]) -> GeneTemplate:
    """Parse one JSON object into a canonical-E ``GeneTemplate``."""
...
def load_gene_templates_from_json(path: str | Path) -> tuple[GeneTemplate, ...]:
    """Load templates from a ``.json`` file or a directory of ``.json`` files."""
...
def gene_template_from_generated_sample(gene: GeneratedSampleGene) -> GeneTemplate:
    """Build canonical-E template from exhaustive sample-gene abstract grid."""
