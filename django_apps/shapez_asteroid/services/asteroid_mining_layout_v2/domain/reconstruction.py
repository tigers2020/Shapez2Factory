...
class ReconstructionDTO:
    ...
    extraction shell ∪ inferred interior **mining-region** cells (empty lattice inside the
    closed hull: ``full_barrier − belt − pipe``, and when a shell exists also minus
    equipment footprint for geometry-only inference; not arbitrary off-map void) ∪
    existing extractor/miner footprint ∪ existing extension footprint, minus **permanent**
    obstacles (belt, pipe, platform, other solid layout kinds). ``extractor_cells`` /
    ``extension_cells`` keep blueprint snapshots; ``equipment_footprint_mineable_cells`` is
    their sorted union (prior mineable evidence). ``full_barrier_cells`` remains the union of
    all occupied blueprint coordinates (not equivalent to permanent mineable blockers).

    ``interior_patch_cells`` are inferred **empty** interior lattice sites only (not
    equipment footprint cells); they are included in the same formal asteroid field as
    mineable placement targets and are not ``RouteZone.INTERNAL_VOID`` / off-map void for
    later passes.
    ...
    **Void topology (STEP 1, Pass1 gating)**: ``external_void_cells`` / ``internal_void_cells``
    ...
    internal_hole_rim_mineable_cells: tuple[BlueprintCell, ...] = ()
