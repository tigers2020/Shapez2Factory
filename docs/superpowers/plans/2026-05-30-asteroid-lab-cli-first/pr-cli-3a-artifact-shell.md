...
- [ ] **Step 1 (TDD):** `test_run_key_safety.py` — traversal/separator/dot rejected; valid key resolves under root.
- [ ] **Step 2 (TDD):** `test_manifest_schema_version.py` — unknown version rejected.
- [ ] **Step 3 (TDD):** `test_validate_artifact.py` — tampered file (hash mismatch) → fail; lifecycle != ARTIFACT_WRITTEN → fail.
- [ ] **Step 4:** Implement CLI shell + `validate-artifact`; `run` returns typed "stack unavailable" until 3b.
- [ ] **Step 5:** Collision policy + `--replace-existing`.
...
- CLI shell + validate-artifact work; run_key/root safety + schema version + collision guards green; core-pure.
