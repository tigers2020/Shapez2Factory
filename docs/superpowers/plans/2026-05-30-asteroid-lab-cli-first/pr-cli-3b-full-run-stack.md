...
- [ ] **Step 1 (SDD):** `test_cli_run_artifact.py` — run on fixture; final dir only after success; manifest hashes valid; all output files present; JSONL parses per line.
...
- [ ] **Step 4 (SDD):** `test_replay_core_monotonic.py` (guard B) + `test_cli_exit_codes.py` (BA-7) + `test_replay_core_no_django_replay.py` (guard E).
