...
Pipeline **contract (A)** is closed. The gap is **commit survivability (B)** within the selection budget: **24 ordered ??20 confirmed**, with **4Ã— `route_probe_failed`** at commit reprobe (not inlet skips on the reference run).
...
|------|--------|-----------|
| `shared_path_pressure_penalty` | `Î± * \|path_cells ??selected_route_cells\|` | **Active** ??`path_cells = frozenset(normalize_probe_path(...))` |
| `route_fragility_penalty` | `Î² * narrow_segment_count` | **0** ??`narrow_segment_count = 0` until RouteClass segment data exists on `GeneCandidate` |

**Constants** (new in `candidate_score.py`, tuned for Phase I scale):

```python
SHARED_PATH_PRESSURE_WEIGHT = 2.0   # Î±
ROUTE_FRAGILITY_WEIGHT = 5.0        # Î² (multiplies 0 in v0)
```
...
| Risk | Mitigation |
|------|------------|
| Over-penalizing trunk merges lowers throughput picks | Conservative Î±; keep throughput weight dominant; gate on reference run |
...
Reference still **21 confirmed / 2 probe_fail / 1 inlet** after T1.2b. Next track: **[`2026-05-22-reprobe-drift-shadow-domain-design.md`](2026-05-22-reprobe-drift-shadow-domain-design.md)** ??Phase I??shadow domain parity; RD-GATE requires **24 selected, 24 confirmed, 0 probe_fail, 0 inlet** on reference (not ?? / ??3).

## Alternatives rejected

| Alternative | Why not |
|-------------|---------|
| Mirror-only (scope A) | Leaves Run #11?™s 4Ã— probe_fail unaddressed |
