...
| 계층 | 정본 (EN) | Wire / 식별자 |
|------|-----------|----------------|
| 제품 | **Lab replay timeline** | `lab_replay_frames_json`, `lab-replay-frames-data`, 단일 scrubber |
| 도메인 DTO | **Replay timeline frame** (`ReplayTimelineFrame`) | `frame_index`, `map_view`, `event_type`, `phase` |
| 합성 | **Replay timeline composition** | `compose_replay_timeline` |
| Lab 소스 | Lab ORM `ReplayTrack` / `ReplayFrame` | ORM 그대로 |
| Runtime 소스 | Solver runtime replay segment | `solver_runtime_replay_frames` in `SolverRun.config_json` |
| **폐기** | dual-track, optimization replay (product), “unified replay” (UX/코드 접두사) | `optimization_replay*`, `optimizationReplayFrameIndex` |

코드·문서·주석에서 **“unified replay” / `unified_*` 심볼 사용 금지** (마이그레이션 완료).
...
**하위 호환:** 구현 전환 기간 `OptimizationReplayEventType` 값 문자열은 위와 **동일 value**를 유지할 수 있다. 제품 DTO 이름은 `ReplayEventType` / `ReplayTimelineFrame`으로 통일한다.
...
optimization·solver 내부 이벤트는 **관측용 로그가 아니라** `ReplayTimelineFrame`으로 기록한다.
...
| `MAX_LAB_REPLAY_TIMELINE_CELLS_PER_FRAME` | 2000 | Lab / unified adapter·composer (9B는 truncate 미적용) |
| `MAX_LAB_REPLAY_TIMELINE_FRAMES` | 500 | 통합 timeline 목표 상한 (9D composer) |
...
| ID | 목표 | 산출 |
|----|------|------|
| **9A** | `ReplayTimelineFrame` DTO + enum + JSON 직렬화 + 계약 단위 테스트 | `django_apps/asteroid_lab/replay/timeline_*.py`, `replay_enums.py` |
| **9B** | Lab `ReplayFrame` / snapshot 이벤트 → `ReplayTimelineFrame` adapter | `phase=decode` / `reconstruction`; 9D baseline |
| **9C** | Optimization 이벤트 → 2D `map_view` adapter | **완료** — `optimization_unified_adapter.py` |
| **9D** | Timeline composer | **완료** — `timeline_composer.py` |
...
**산출:** `django_apps/asteroid_lab/replay/lab_timeline_adapter.py`, `replay_event_coverage.py`, `replay_limits.py`
...
**테스트:** `tests/unit/asteroid_lab/test_lab_timeline_adapter.py`, `test_replay_event_coverage_matrix.py`, `test_replay_limits.py`

### 9C — Optimization adapter (구현 완료)

**산출:** `django_apps/asteroid_lab/replay/optimization_unified_adapter.py`, `projection_context.py`, `replay_recording_cells.py`, `replay_event_coverage.SUPPORTED_BY_9C_OPTIMIZATION_ADAPTER`
...
**산출:** `django_apps/asteroid_lab/replay/timeline_composer.py` — `compose_replay_timeline`
...
[x] `MAX_LAB_REPLAY_TIMELINE_FRAMES` 초과 시 keyframe+tail truncate + 마지막 프레임 `replay_truncated` / `truncation_reason` / `dropped_frame_count`
...
**테스트:** `test_timeline_composer.py` (`test_replay_head_truncate_retains_result_layout`, `test_replay_truncation_retains_first_reconstruction_keyframe` 포함)
...
```text
[x] 제품 replay = `lab_replay_frames_json` (Lab ORM + persisted optimization, `ReplayTimelineFrame` JSON, `map_view` 포함)
...
[ ] scrubber가 replay timeline만 제어 (두 번째 optimization controller 없음)
...
기존 회귀: `test_manual_snapshot_replay_not_used_as_algorithm_input_doc`, `test_lab_js_replay_wiring_smoke` — **replay timeline** 기준으로 갱신 예정.
