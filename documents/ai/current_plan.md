...
**Status (2026-05-30)**: **Roadmap v0.2 repurposed — Algorithm Stabilization (v0.2A Core Recovery)**. **v0.1 CLOSED** (architecture + partial fixtures; product-grade algorithm correctness **not** proven). Board: [`docs/superpowers/2026-05-24-asteroid-lab-catalog-rttp-roadmap.md`](../../docs/superpowers/2026-05-24-asteroid-lab-catalog-rttp-roadmap.md). **Recovery canon:** [`docs/superpowers/specs/2026-05-30-rttp-v0-2-core-algorithm-recovery-design.md`](../../docs/superpowers/specs/2026-05-30-rttp-v0-2-core-algorithm-recovery-design.md).
...
**EVTC (2026-05-26)** — Spec: [`docs/superpowers/specs/2026-05-26-rttp-external-void-transport-capacity-contract.md`](../../docs/superpowers/specs/2026-05-26-rttp-external-void-transport-capacity-contract.md) · Plan: [`docs/superpowers/plans/2026-05-26-rttp-external-void-transport-capacity-contract.md`](../../docs/superpowers/plans/2026-05-26-rttp-external-void-transport-capacity-contract.md). **EVTC-1a/1b CLOSED.** **EVTC-2–7:** seven commits on `feat/decontamination-pr-f2-asteroid-lab` (planner → validation → weighted probe → docs). **Narrow regression:** `pytest -k "rttp or evtc or exterior_connector or route_probe"` green (236). **EVTC-6b DEFERRED:** `route_not_shortest_feasible` code only; no validation wiring. **A5 recovery baseline (post-EVTC):** `committed >= 22` (was 24 pre-EVTC weighted exterior-void probe) — intentional capacity-aligned route choice, not recovery truncation; re-validate via `capture_rttp_recovery_evidence` before PR. **Release:** EVTC commit train done · **`test_fast.ps1` green (2026-05-30)** · evidence [`after-evtc.json`](../../docs/superpowers/reports/2026-05-30-rttp-core-recovery-evidence-after-evtc.json) refreshed (22 extractors, `validation_passed=true`). **EVTC-2–7 CLOSED** (narrow + fast gate). **PR HOLD:** repo-wide `mypy` / `test_full.ps1` per AGENTS PR gate unless scoped. **EVTC-6b DEFERRED** unchanged. A6 CLOSED.

**ACTIVE (parallel) — RTTP v0.2 Core Algorithm Recovery** — extension topology (S2b) · Gate B throughput · PR-F3–F5 remain **HOLD** until product goals met. Spec: [`docs/superpowers/specs/2026-05-30-rttp-v0-2-core-algorithm-recovery-design.md`](../../docs/superpowers/specs/2026-05-30-rttp-v0-2-core-algorithm-recovery-design.md).

**CLOSED (2026-05-26):** **A2 Catalog extension topology (A2-H1)** — S2a topology-drop fixed (`_extension_offsets_from_footprint`, INV-R-03). **S2b catalog/source absence** deferred (2-cell colinear DB only). After-A2: [`2026-05-30-rttp-core-recovery-evidence-after-a2.json`](../../docs/superpowers/reports/2026-05-30-rttp-core-recovery-evidence-after-a2.json). **A2b options (post-A3):** richer game_data extension entities · pattern synthesis · `Layout_*MinerExtension` equipment family.

**CLOSED (2026-05-26):** **A3 Output transport + exterior route materialization** — `_augment_route_cells_with_output_spine` in `incremental_commit`; full trunk in route-domain replay overlay. After-A3: [`2026-05-30-rttp-core-recovery-evidence-after-a3.json`](../../docs/superpowers/reports/2026-05-30-rttp-core-recovery-evidence-after-a3.json). Gate A minimal pass; commit regression fixed in A3.1.

**CLOSED (2026-05-26):** **Task 3.5 — Route spine conflict diagnosis** (evidence-only). Report: [`docs/superpowers/reports/2026-05-30-rttp-task-35-spine-conflict-diagnosis.md`](../../docs/superpowers/reports/2026-05-30-rttp-task-35-spine-conflict-diagnosis.md).

**CLOSED (2026-05-26):** **A3.1 Route spine sharing / minimal reservation** — trunk overlap allowed; spine stops at private committed routes. After-A3.1: [`2026-05-30-rttp-core-recovery-evidence-after-a31.json`](../../docs/superpowers/reports/2026-05-30-rttp-core-recovery-evidence-after-a31.json). Primary: `committed_extractor_count` **24** (was 10 after-A3); `route_cell_conflict` delta **0** vs spine-OFF; Gate A still pass. Report: [`2026-05-30-rttp-task-31-spine-sharing.md`](../../docs/superpowers/reports/2026-05-30-rttp-task-31-spine-sharing.md).

**CLOSED (2026-05-26):** **Task 4 — A4 placement goal coverage** — product `placement_goal_count = ceil(asteroid_field_cell_count × placement_target_percent / 100)` from **`ReconstructionCompleteMap`**; diagnostic flag `placement_goal_shortfall` (not `placement_goal_capped`). After-A4: [`2026-05-30-rttp-core-recovery-evidence-after-a4.json`](../../docs/superpowers/reports/2026-05-30-rttp-core-recovery-evidence-after-a4.json). Plan: [`2026-05-30-rttp-task-4-placement-goal-coverage.md`](../../docs/superpowers/plans/2026-05-30-rttp-task-4-placement-goal-coverage.md).

**CLOSED (2026-05-26):** **Task 5 — A5 commit / reservation hardening** — FL-06: reserve `output_stub` when FOT is outside mineable even if stub is in `blocked_cells` (probe envelope only). `ThroughputShortfallReason` renamed to `*_shortfall` tokens. After-A5: [`2026-05-30-rttp-core-recovery-evidence-after-a5.json`](../../docs/superpowers/reports/2026-05-30-rttp-core-recovery-evidence-after-a5.json). Primary: `committed_extractor_count` **62** (was 24 after-A4), `placement_goal_count=467`, `output_stub_not_reserved` **0**, Gate A pass. Tests: `tests/unit/asteroid_lab/test_rttp_a5_output_stub_reservation.py`.

**CLOSED (2026-05-26):** **Task 6 — A6 validation fail-closed** — read-only `validate_layout_connectivity_issues`; issue codes `missing_output_transport` / `missing_exterior_route`; `committed < placement_goal` stays diagnostic (`placement_goal_shortfall`, `throughput_target_shortfall`), not validation failure. After-A6: [`2026-05-30-rttp-core-recovery-evidence-after-a6.json`](../../docs/superpowers/reports/2026-05-30-rttp-core-recovery-evidence-after-a6.json). Primary: `validation_passed=true`, `gate_a_passed=true`, 62/467 commits. Tests: `test_rttp_layout_connectivity_validation.py`, `test_rttp_core_recovery_gate_a.py`.

**CLOSED (2026-05-26):** **A1 Stage diagnosis** — `rttp_recovery_stage_diagnosis.py`; evidence rows emit `first_failing_stage`, `blocking_stages`, `diagnostic_flags`, `primary_symptom`. Primary baseline: **S2** first, flags `missing_extensions` + `route_materialization_missing` + `validation_false_positive`; symptom `route_cells_zero_but_validation_passed`. Solver semantics unchanged.

**CLOSED (2026-05-26):** **A0 Evidence Freeze** — evidence tooling only (no solver semantics). Baseline: [`docs/superpowers/reports/2026-05-30-rttp-core-recovery-evidence-baseline.json`](../../docs/superpowers/reports/2026-05-30-rttp-core-recovery-evidence-baseline.json) · [`2026-05-30-rttp-core-recovery-evidence-baseline.md`](../../docs/superpowers/reports/2026-05-30-rttp-core-recovery-evidence-baseline.md). Signature: 23 extractors, 0 route/exterior, `validation_passed=true` on primary slugs (Gate A fail); FOT overlay count 14 without `route.committed_path` rows.

**HOLD:** **Decontamination PR-F3–F5** until Gate A materialization evidence on recovery slug (see recovery spec §8). F3 branch `feat/decontamination-pr-f3-game-data` — do not execute. Plan (frozen): [`docs/superpowers/plans/2026-05-30-test-cleanup-aggressive-decontamination-pr-f.md`](../../docs/superpowers/plans/2026-05-30-test-cleanup-aggressive-decontamination-pr-f.md).
...
**HOLD (follow-on):** **Decontamination PR-F3–F5** — gated deletes only via `PR_F_*` registry; resume after core recovery Gate A. F1 **SKIP** (0 mechanical rows). Standing gate: `scripts/test_quarantine_registry.ps1`. Spec: [`docs/superpowers/specs/2026-05-30-test-cleanup-aggressive-decontamination-design.md`](../../docs/superpowers/specs/2026-05-30-test-cleanup-aggressive-decontamination-design.md).
