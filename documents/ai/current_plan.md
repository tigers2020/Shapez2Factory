...
**EVTC (2026-05-26)** — Spec · Plan (links above). **EVTC-1a/1b CLOSED.** **EVTC-2–7 CLOSED** (code + narrow RTTP/EVTC + `test_fast.ps1` + evidence recapture). **Commit train:** 9 commits on `feat/decontamination-pr-f2-asteroid-lab` (`dae26432`…`e3453162`). **Narrow:** `pytest -k "rttp or evtc or exterior_connector or route_probe"` — 236 passed. **Fast gate:** `scripts/test_fast.ps1` — 1612 passed (2026-05-30). **Evidence:** [`after-evtc.json`](../../docs/superpowers/reports/2026-05-30-rttp-core-recovery-evidence-after-evtc.json) — `committed_extractor_count=22`, `validation_passed=true`, `gate_a_passed=false` (diagnostic; not a release-close blocker). **A5 baseline 24→22:** EVTC weighted exterior-void probing prioritizes capacity-aligned exterior connector routes over legacy ring-port candidates; layout passes validation with 22 committed extractors; Gate A remains a diagnostic failure as expected. **EVTC-6b DEFERRED:** `route_not_shortest_feasible` — diagnostic code exists; validation wiring and enforcement are **not** part of this release (do not claim “verified” or “EVTC-6b closed” in release notes). **PR/push HOLD:** no explicit push/PR request; pre-merge needs `test_full.ps1` + mypy triage; **A2–A6 parallel WIP must not ship in the EVTC PR** (stash or separate branch before push).

**ACTIVE (parallel) — RTTP v0.2 Core Algorithm Recovery** — extension topology (S2b) · Gate B throughput · PR-F3–F5 remain **HOLD** until product goals met. Spec: [`docs/superpowers/specs/2026-05-30-rttp-v0-2-core-algorithm-recovery-design.md`](../../docs/superpowers/specs/2026-05-30-rttp-v0-2-core-algorithm-recovery-design.md) (**implementation checklist §13** — Gate A CLOSED 2026-05-26, Gate B OPEN).
