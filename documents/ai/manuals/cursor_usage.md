...
4. 구현
5. 검증(`pytest` 구간 → 필요 시 전체, `ruff`/`mypy`/`black`)
6. 반복
...
에이전트 시대에는 테스트·린트·타입체크를 **짧은 주기로 많이** 돌리게 되므로, **느린 스위트·전체 pytest 남발**이 비용과 대기 시간을 키운다. 구간 실행·마커·경로 실행을 습관화한다 ([`testing.md`](testing.md), [`AGENTS.md`](../../../AGENTS.md) 테스트 표).
...
| 검증 게이트 | invariant·최종 검증, `pytest`→`ruff`→`mypy`→`black` |
