...
**역할:** Refactor Lead 지시에 따른 **읽기 전용** 감사 산출물이다. 코드 변경 없음.  
...
| **evidence** | `solver/recovery_policy.py` `synthesize_recovery_validation_outcome`: `return_reason == "ok"`일 때 `out["commit_reason"] = str(summary.get("pass3_commit_reason") or "validation_ok")` (약 160–164행). |
...
| **evidence** | `solver_pipeline/recovery_orchestrator.py` `run_solver_timeline_pipeline`: STEP4 후 `routing_snapshot` 고정으로 **루프: Pass3 → P4 → finalize** (약 339–408행대). `validation_recovery` 시 `RECOVERY_BRANCH` 이벤트(`phase`: `validation_recovery`). |
...
- `rg` 패턴: `recovery_trigger`, `commit_reason`, `recovery_trigger_reason`, `run_solver_timeline_pipeline`, `cascade_corrective`, `validation_recovery`, `latest.ndjson`, `open(` in `solver_trace.py`.
...
- 본 파일을 기준으로 **Phase 1 PR** 범위를 Epic B TODO로 쪼갠다.
- `git commit`으로 문서 기준선 고정 후 브랜치/PR에서 구현 시작.
