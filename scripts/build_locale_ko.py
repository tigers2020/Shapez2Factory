...
STRICT_LOCALE_PY = (BASE / "django_apps" / "web" / "views" / "public_pages.py",)
...
KO: dict[str, str] = {
    ...
    # copy decode / bbox msgids (public_pages 등 리터럴과 일치)
    ...
    "Invalid blueprint copy code.": "유효하지 않은 SHAPEZ2 복사 코드입니다.",
    ...
    "terrain upper bound": "지형 상한",
    "terrain upper bound (shape)": "지형 상한 (shape)",
    "terrain upper bound (fluid)": "지형 상한 (fluid)",
    "confirmed / map cells": "확정 / 맵 셀",
    "Shape": "Shape",
    "Fluid": "Fluid",
    ...
    "Confidence": "신뢰도",
    "Cells": "셀",
    "Confirmed mineable": "확정 채굴 가능",
    "Ambiguous": "모호",
    "External void": "외부 void",
    "Theoretical max (shape)": "이론 최대치 (shape)",
    "Theoretical max (fluid)": "이론 최대치 (fluid)",
    "Platform upper bound": "플랫폼 상한",
    "Rule source": "규칙 출처",
    "Capacity basis": "용량 기준",
    "Confirmed placements": "확정 배치",
    "Validation": "검증",
    "Candidate count": "후보 수",
    "Commit order": "커밋 순서",
    "Actual output": "실제 출력",
    "Target throughput": "목표 처리량",
    "Target percent": "목표 비율",
    "% of theoretical max": "이론 최대치 대비 %",
    "Target satisfied": "목표 달성",
    "Short by": "부족",
    "Utilization": "활용률",
    "Budget status": "예산 상태",
    "Throughput shortfall": "처리량 부족",
    "throughput target shortfall": "목표 처리량 미달",
    "actual output unavailable": "실제 출력 없음",
}

# djangojs domain: legacy JS + React msgids (English -> Korean). Keys must match gettext/shapezUiT/t().
KO_JS: dict[str, str] = {
    # Asteroid Lab PR-2a — shapezUiT
    "actual output pending": "실제 출력 대기 (PR-2b)",
    "terrain upper bound (shape)": "지형 상한 (shape)",
    "terrain upper bound (fluid)": "지형 상한 (fluid)",
    "placement(s)": "배치",
    ...
    "Short by": "부족",
    "throughput target shortfall": "목표 처리량 미달",
    "RTTP validation failed": "RTTP 검증 실패",
    "actual output unavailable": "실제 출력 없음",
    "/min": "/분",
    "Belt": "벨트",
    "Fluid": "유체",
