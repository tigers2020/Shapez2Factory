/**
 * Playwright PNG render for a single preview_scene (typically one cell).
 * Same pipeline as render_graph_preview.mjs — scene JSON matches shape preview serialization.
 * Output PNG matches graph-preview-render.html viewport CSS (512×512).
 *
 * Usage: node scripts/render_part_sprite.mjs --scene-file <path> --out <path.png>
 */
...
async function main() {
  ...
  try {
    // graph-preview-render.html: .frame is 672px; viewport must contain centered layout.
    ...
    const omitBackground = Boolean(sceneJson.transparent_background);
    await preview.screenshot({ path: outputPath, omitBackground });
  } finally {
    await browser.close();
    await server.close();
  }
