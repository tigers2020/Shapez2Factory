...
def test_purge_narrow_keeps_manual_and_removes_stale_v1() -> None:
    ...
    assert not GeneSeed.objects.filter(gene_key=gene_key_for_rank(1)).exists()
    assert (
...
def test_purge_narrow_removes_extra_v2_key() -> None:
    ...
    call_command("seed_miner_patterns", purge_non_seed=True)
    assert not GeneSeed.objects.filter(gene_key="miner_seed_m9e_99").exists()
    assert GeneSeed.objects.get(gene_key=gene_key_for_pattern_id("m0e_01"))
