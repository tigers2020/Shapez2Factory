...
def test_domain_to_react_flow_target_shape_node_type() -> None:
    """target ??•  shape??React Flow ?¸ë“œ type ``output`` ?¼ë¡œ ë§¤í•‘?œë‹¤(?£ì? ?†ì´ ?€?…ë§Œ ê²€ì¦?."""
    raw = {
        "schema_version": 1,
        "nodes": [
            {
...
def test_react_flow_round_trip_preserves_fractional_positions() -> None:
    """ìº”ë²„???œë˜ê·?ì¢Œí‘œ??graph_document x,y??ë³´ì¡´?œë‹¤(?€?¥Â·ì¬ë¡œë“œ ?•í•©)."""
    raw = {
        "schema_version": 1,
        "nodes": [
            {
...
def test_domain_to_react_flow_two_output_source_handles() -> None:
    """?¤ì¶œ???°ì‚°?€ React Flow ``sourceHandle``???ˆì¸ë³„ë¡œ ?˜ëˆˆ??"""
    raw = {
        "schema_version": 1,
        "nodes": [
...
def test_domain_to_react_flow_binary_input_handles_follow_slot_not_list_order() -> None:
    """Painter ???¬ë¡¯???ˆëŠ” ?…ë ¥?€ edges ë°°ì—´ ?œì„œ?€ ë¬´ê??˜ê²Œ ``targetHandle``??ë°˜ì˜?œë‹¤."""
    raw = {
        "schema_version": 1,
        "nodes": [
            {
