...
def test_node_cumulative_overlay_retains_first_candidate_cells() -> None:
    ...
    script = textwrap.dedent("""
        ...
        const mk = (et, cells) => ({
          ...
          event_type: et,
          metric_snapshot_json: { server_xy_params: [2, 0] },
          cell_overlay_json: {
            cells: cells.map((c) =>
        ...
        const frames = [
          ...
          mk("route_probe.succeeded", [{ x: 0, y: 0, overlay_role: "route_path" }]),
        ];
        const out = A.projectCumulative(frames, 0, 2, resolve, labRows);
...
def test_node_rejected_marks_only_reject_frame_cells() -> None:
    ...
    script = textwrap.dedent("""
        ...
        const frames = [
          { phase: "decode", frame_key: "reconstruction_x" },
          mk("candidate.generated", [{ x: 1, y: 0, overlay_role: "candidate_occupied" }]),
          mk("candidate.generated", [{ x: 0, y: 0, overlay_role: "candidate_occupied" }]),
          mk("candidate.rejected", [{ x: 0, y: 0, overlay_role: "candidate_occupied" }]),
        ];
        const out = A.projectCumulative(frames, 0, 3, resolve, labRows);
        const by = new Map(out.directives.map((d) => [d.domIndex, d]));
...
def test_node_rolled_back_overlays_previous_committed_tone() -> None:
    ...
    script = textwrap.dedent("""
        ...
        const mk = (et, cells) => ({
          ...
          frame_key: "optimization_00_x",
          event_type: et,
          metric_snapshot_json: { server_xy_params: [1, 0] },
          cell_overlay_json: {
            cells: cells.map((c) =>
              Object.assign({ cell_kind: "optimization_overlay", severity: "info" }, c),
            ),
        ...
        const frames = [
          { phase: "decode", frame_key: "reconstruction_x" },
          mk("route.committed", [{ x: 0, y: 0, overlay_role: "route_path" }]),
          mk("route.rolled_back", [{ x: 0, y: 0, overlay_role: "route_path" }]),
        ];
        const out = A.projectCumulative(frames, 0, 2, resolve, labRows);
