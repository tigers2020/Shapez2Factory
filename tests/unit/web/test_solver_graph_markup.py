...
def test_materialized_graph_uses_current_graph_markup() -> None:
    from django.test import Client

    response = Client().post("/api/solver/solve/", data={"code": "CuRuSuSu"})

    assert response.status_code == 200
    payload = response.json()
    materialized_graph = payload["materialized_graph"]

    rendered = _render_graph_markup(cast(dict[str, object], materialized_graph))
    html = rendered["html"]
